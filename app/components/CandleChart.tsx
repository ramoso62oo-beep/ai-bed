...
const RANGES: [string,string][] = [["1","24H"],["7","7J"],["30","1M"],["90","3M"],["365","1A"]];
...
export default function CandleChart({ pair="BTC" }: { pair?: string }) {
  ...
  // Liste réelle des cryptos (symbol → id CoinGecko)
  useEffect(() => {
  ...
  // Charge les vraies bougies + prix live
  const loadData = useCallback(async () => {
    const id = idForSym(sym);
    if (!id) return;
    try {
      const d = await fetch(`/api/ohlc?id=${id}&days=${days}`).then(r=>r.json());
      let candles: Candle[] = d.candles || [];
      // Met à jour la dernière bougie avec le prix live
      if (d.price && candles.length) {
        const last = { ...candles[candles.length-1] };
        last.c = d.price; last.h = Math.max(last.h, d.price); last.l = Math.min(last.l, d.price);
  ...
  useEffect(() => { setLoading(true); loadData(); const id = setInterval(loadData, 20000); return ()=>clearInterval(id); }, [loadData]);
  ...
  useEffect(() => {
    ...
    function draw(){
      ...
      const n=candles.length, slot=cw/n, bodyW=Math.max(slot*0.6,1.5);
  ...
  const symbols = coins.length ? coins.map(c=>c.symbol) : ["BTC","ETH","SOL","BNB","XRP"];
    ...
    <div className="panel-3d" style={{ background:"rgba(6,13,46,0.55)", border:"1px solid rgba(10,26,92,0.6)", borderRadius:10, overflow:"hidden", flexShrink:0 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"9px 13px", borderBottom:"1px solid rgba(10,26,92,0.6)", flexWrap:"wrap", gap:6 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ display:"flex", alignItems:"center", gap:4, fontSize:".54rem", color:"var(--green)" }}><span style={{ width:5, height:5, borderRadius:"50%", background:"var(--green)", animation:"pulse-red 1.5s infinite" }}/>réel</span>
        </div>
        <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", gap:2 }}>{RANGES.map(([d,lb])=>(<button key={d} onClick={()=>setDays(d)} style={{ padding:"3px 7px", borderRadius:5, fontSize:".56rem", fontWeight:700, cursor:"pointer", border:"none", background:days===d?"rgba(192,57,43,0.2)":"transparent", color:days===d?"var(--red)":"var(--muted)" }}>{lb}</button>))}</div>
        </div>
      </div>
      {stats.last>0 && (
      ...
      <div style={{ position:"relative" }}>
        {loading && <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:".7rem", color:"var(--muted)" }}>Chargement des données réelles…</div>}

...
export default function Tooltip({ text, children }: { text: string; children: React.ReactNode }) {
  ...
  return (
    <span ref={ref} onMouseEnter={enter} onMouseLeave={() => setShow(false)} style={{ display:"inline-flex", position:"relative" }}>
      ...
      {show && (
        <span style={{
          position:"fixed", left:coords.x, top:coords.y - 8, transform:"translate(-50%,-100%)",
          ...
          {text}
          <span style={{ position:"absolute", left:"50%", bottom:-5, transform:"translateX(-50%)", width:9, height:9,
            background:"rgba(4,7,26,0.97)", borderRight:"1px solid rgba(74,111,165,0.4)", borderBottom:"1px solid rgba(74,111,165,0.4)",
            rotate:"45deg" }}/>
        </span>
      )}
    </span>

...
export default function AdminPage(){
  ...
  const view=users.filter(u=> !q || (u.email||"").toLowerCase().includes(q.toLowerCase()) || (u.full_name||"").toLowerCase().includes(q.toLowerCase()) || (u.memberId||"").toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div className="dash-root" style={{ display:"grid", gridTemplateColumns:"210px 1fr", height:"100vh", background:"var(--navy)", overflow:"hidden" }}>
      ...
      <div style={{ overflowY:"auto", padding:"78px 28px 40px", position:"relative", zIndex:1 }}>
        ...
        {/* Stats */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:18, maxWidth:560 }}>
          {[["Comptes total",stats.total,"white"],["Abonnés payants",stats.paid,"var(--green)"],["Sans abonnement",stats.free,"var(--muted2)"]].map(([k,v,c])=>(
            <div key={String(k)} style={{ ...card, padding:"14px 16px" }}>
              <div style={{ fontSize:".6rem", color:"var(--muted)", textTransform:"uppercase", letterSpacing:".06em" }}>{k}</div>
              <div style={{ fontSize:"1.5rem", fontWeight:900, color:c as string, fontFamily:"var(--font-orbitron,monospace)" }}>{v}</div>
            </div>
          ))}
        </div>

        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="🔎 Rechercher (email, nom, ID…)" style={{ width:"100%", maxWidth:400, background:"rgba(4,7,26,0.6)", border:"1px solid rgba(74,111,165,0.3)", borderRadius:9, padding:"9px 14px", color:"white", fontSize:".76rem", outline:"none", marginBottom:14 }} />
        ...
        <div style={{ ...card, overflow:"hidden" }}>
          ...
          <div style={{ maxHeight:"calc(100vh - 360px)", overflowY:"auto" }}>
            {!loading && view.length===0 && <div style={{ padding:"20px", color:"var(--muted)", fontSize:".76rem" }}>Aucun compte.</div>}
            {view.map(u=>(
